...
#define UPDATE_STATE_4_offset(X, offset) \
    tmp[0] = xor(state[(XOR_1+offset)&MOD], state[(XOR_2+offset)&MOD]); \
    tmp[0] = xor(tmp[0], state[(XOR_3+offset)&MOD]); \
    tmp[1] = xor(state[(XOR_1+1+offset)&MOD], state[(XOR_2+1+offset)&MOD]); \
    tmp[1] = xor(tmp[1], state[(XOR_3+1+offset)&MOD]); \
    tmp[2] = xor(state[(XOR_1+2+offset)&MOD], state[(XOR_2+2+offset)&MOD]); \
    tmp[2] = xor(tmp[2], state[(XOR_3+2+offset)&MOD]); \
    tmp[3] = xor(state[(XOR_1+3+offset)&MOD], state[(XOR_2+3+offset)&MOD]); \
    tmp[3] = xor(tmp[3], state[(XOR_3+3+offset)&MOD]); \
    state[offset&MOD] = aesemc(tmp[0], X); \
    state[(offset+1)&MOD] = aesemc(tmp[1], X); \
    state[(offset+2)&MOD] = aesemc(tmp[2], X); \
    state[(offset+3)&MOD] = aesemc(tmp[3], X); \
    state[(XOR_8+offset)&MOD] = aesdimc(state[(XOR_8+offset)&MOD], X); \
    state[(XOR_8+1+offset)&MOD] = aesdimc(state[(XOR_8+1+offset)&MOD], X); \
    state[(XOR_8+2+offset)&MOD] = aesdimc(state[(XOR_8+2+offset)&MOD], X); \
    state[(XOR_8+3+offset)&MOD] = aesdimc(state[(XOR_8+3+offset)&MOD], X);


/*
 * This function was prepared for initialize performance and rest encrypt/decrypt
 * For a rest with 4x blocks, the unroll can be used with a finalize shift
 * Have not used in current implement yet
 */
...
/*
 * Update the state with STATE id offset
 * for the unroll design with not shift
 * that we can do 16 update to let it round back
 */
...
/*
 * extra keystream from STATE to get the chiper or message
 */
...
/*
 * extra XOR 1-3 for state update
 * extra XOR 4-5 for encrypt/decrypt
 */
#define XOR_1_5_4STEP_FUSION_OFFSET(offset) \
...
/*
 * update 4 new arrival state to replace the departured state
 */
...
/*
 * cover the XOR 8 position with aesdimc for safety
 */
...
/*
 * encrypt 4 blocks with unroll
 */
#define ENCRYPT_4_OFFSET(M, C, offset) \
    LOAD_4STEP_OFFSET(M, offset) \
    XOR_1_5_4STEP_FUSION_OFFSET(offset) \
...
void HAE_stream_init(DATA128b* state, const uint8_t *key, const uint8_t *iv) {
    ...
    DATA128b temp, tmp[16];
    for (size_t i = 0; i < 2; ++i) {
        UPDATE_STATE_4_offset(const_0, 0);
        UPDATE_STATE_4_offset(const_0, 4);
        UPDATE_STATE_4_offset(const_0, 8);
        UPDATE_STATE_4_offset(const_0, 12);
    }
...
void HAE_stream_finalize(DATA128b* state, uint64_t ad_len, uint64_t plain_len, uint8_t *tag) {
    ...
    DATA128b temp, tmp[16];
    for (size_t i = 0; i < 2; ++i) {
        UPDATE_STATE_4_offset(xor(lead, lemsg), 0);
        UPDATE_STATE_4_offset(xor(lead, lemsg), 4);
        UPDATE_STATE_4_offset(xor(lead, lemsg), 8);
        UPDATE_STATE_4_offset(xor(lead, lemsg), 12);
    }
    temp = state[0];
